...
async function scrapeLawyerProfile(url) {
  try {
      ...
      if (scoreSpan.length > 0) {
        ...
        if (scoreSpan.length > 1) {
          ...
          if (secondScore && secondScore.match(/[0-9]+/)) {
            rating_count = secondScore.includes('+') ? secondScore : secondScore + '+';
          }
        }
    ...
    if (aboutParagraphs.length === 0) {
      console.log("No about paragraphs found in box-card, trying profile-summary");
    ...
    $('div.box-card').each((_, el) => {
      ...
      if (heading && (heading.includes('Specialization') || heading.includes('Expertise'))) {
        ...
        console.log("Extracted specialization text:", specializationText.substring(0, 100) + "...");
      }
    });
      ...
      if (heading && heading.includes('Court')) {
        ...
        if (courts.length === 0) {
          ...
          if (text) {
            ...
            if (text.includes(',')) {
              const courtItems = text.split(',').map(t => t.trim()).filter(t => t);
              courts.push(...courtItems);
              console.log("Found courts in comma-separated text:", courtItems.length, "courts");
            } else {
              courts.push(text);
      ...
      if (heading && (heading.includes('Review') || heading.includes('Testimonial'))) {
        ...
        $(el).find('.review-box, .review-item, .testimonial').each((_, review) => {
          ...
          if (name || review_text) {
            ...
            console.log("Found review from:", name, "verified:", verified_client);
          }
        });
      }
    });
    ...
    if (name === expectedData.name) {
      ...
      if (!is_verified && expectedData.is_verified) lawyerData.is_verified = expectedData.is_verified;
      if (!img_url && expectedData.img_url) lawyerData.img_url = expectedData.img_url;
      if (!rating && expectedData.rating) lawyerData.rating = expectedData.rating;
      if (!rating_count && expectedData.rating_count) lawyerData.rating_count = expectedData.rating_count;
      if (!contact_number && expectedData.contact_number) lawyerData.contact_number = expectedData.contact_number;
      if (!location && expectedData.location) lawyerData.location = expectedData.location;
      if (!experience && expectedData.experience) lawyerData.experience = expectedData.experience;
      
      // Arrays - use expected data if our arrays are empty
      if (languages.length === 0 && expectedData.languages && expectedData.languages.length > 0) {
        lawyerData.languages = expectedData.languages;
      }
      
      if (practice_areas.length === 0 && expectedData.practice_areas && expectedData.practice_areas.length > 0) {
      ...
      if (specialization.length === 0 && expectedData.specialization && expectedData.specialization.length > 0) {
      ...
      if (questions_answered.length === 0 && expectedData.questions_answered && expectedData.questions_answered.length > 0) {
        ...
        if (Array.isArray(expectedData.questions_answered[0])) {
          lawyerData.questions_answered = expectedData.questions_answered.map(qa => ({
            question: qa[0] || '',
            details: qa[1] || '',
            answer: qa[2] || ''
          }));
